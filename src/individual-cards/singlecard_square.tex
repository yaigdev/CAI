...
\newcommand{\drawbackground}{
	...
	\fill[cardbg] (-0.12in, -0.12in) rectangle +(2.44in, 2.44in);
	\fi
	% background only to cut area
	\fill[cardbg] (0in, 0in) rectangle +(2.2in, 2.2in);

	\ifdefined\LINES
	% guideline - cut area
	\draw[green] (0in, 0in) rectangle +(2.2in, 2.2in);
	% guideline - safe area
	%\draw[red] (0.12in, 0.12in) rectangle +(1.96in, 1.96in);
	\fi
}

\newcommand{\drawcardicon}{
	\node [above right] at (0in + 0.12in, 2.2in - 0.12in) {\cardicon};
}
...
\newcommand{\drawcardtitle}{
	...
	\node [cardfg,below right] at (0.12in, 0.72in) {\titlefont Infrastructure};
}
